...
fn binary_processes_one_request_per_line() {
    ...
    writeln!(
        input,
        r#"{{"protocolVersion":"0.4.0","requestId":"1","command":"hello","payload":{{}}}}"#
    )
    .unwrap();
    writeln!(
        input,
        r#"{{"protocolVersion":"0.4.0","requestId":"2","command":"state.get","payload":{{}}}}"#
    )
    .unwrap();
    drop(input);
...
fn concurrent_reads_are_answered_by_request_id_in_any_order() {
    ...
    for (id, command, payload) in [
        ...
        writeln!(
            input,
            r#"{{"protocolVersion":"0.4.0","requestId":"{id}","command":"{command}","payload":{payload}}}"#
        )
        .unwrap();
    }
    drop(input);
